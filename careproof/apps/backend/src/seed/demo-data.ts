...
export async function seedDemoData(connection: Connection) {
  ...
  const medicationIds = {
    metformin: new Types.ObjectId(),
    lisinopril: new Types.ObjectId(),
    insulin: new Types.ObjectId(),
    acetaminophen: new Types.ObjectId(),
    davidAtorvastatin: new Types.ObjectId(),
    helenDonepezil: new Types.ObjectId(),
    samuelWarfarin: new Types.ObjectId(),
  };
  const insulinApprovalId = new Types.ObjectId();

  await connection.collection('nurseApprovals').insertMany([
    {
      _id: insulinApprovalId,
      agencyId,
      ...
      visitType: 'Medication review: Insulin Glargine pen',
      priority: 'critical',
      status: 'pending_review',
      nurseNotes: 'High-risk refrigerated insulin requires nurse review before family-visible medication update.',
      reviewedBy: null,
      reviewedAt: null,
      deletedAt: null,
      createdAt: daysAgo(now, 0, 7, 0),
      updatedAt: daysAgo(now, 0, 7, 0),
    },
    {
      agencyId,
      branchId: northsideBranchId,
  ...
  await connection.collection('inspectionFindings').insertMany([
    ...
    },
    {
      agencyId,
      branchId: northsideBranchId,
      ruleId: seededRules[3]._id,
      sourceMedicationId: medicationIds.acetaminophen,
      ...
      clientName: 'Maria Johnson',
      caregiverName: 'Ana Smith',
      description: 'Acetaminophen 500mg tablet expired yesterday and must be removed from active availability.',
      assignedTo: 'Nurse',
      dueDate: now.toISOString().split('T')[0],
      resolvedAt: null,
      deletedAt: null,
      createdAt: daysAgo(now, 0, 8, 0),
      updatedAt: daysAgo(now, 0, 8, 0),
    },
    {
      agencyId,
      branchId: northsideBranchId,
      ruleId: seededRules[3]._id,
      sourceMedicationId: medicationIds.insulin,
      title: 'High-risk medication missing nurse review',
      severity: 'critical',
      status: 'open',
      clientId: clients[0]._id,
      visitId: todayVisits[0]._id,
      ...
      clientName: 'Maria Johnson',
      caregiverName: 'Ana Smith',
      description: 'Insulin Glargine pen is high risk and requires nurse review before the visit is considered medication-ready.',
      assignedTo: 'Nurse',
      dueDate: now.toISOString().split('T')[0],
      resolvedAt: null,
      deletedAt: null,
      createdAt: daysAgo(now, 0, 8, 30),
      updatedAt: daysAgo(now, 0, 8, 30),
    },
  ]);
  ...
  const yesterday = daysAgo(now, 1, 0, 0).toISOString().split('T')[0];
  const twentyDaysFromNow = new Date(Date.now() + 20 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  const thirtyDaysFromNowForMedication = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  const fortyFiveDaysFromNow = new Date(Date.now() + 45 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  const ninetyDaysFromNowForMedication = new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  const oneHundredTwentyDaysFromNow = new Date(Date.now() + 120 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];

  await connection.collection('medicationRecords').insertMany([
    {
      _id: medicationIds.metformin,
      agencyId,
      branchId: northsideBranchId,
      clientId: clients[0]._id,
      visitId: todayVisits[0]._id,
      ...
      pharmacyName: 'Silver Spring Family Pharmacy',
      startDate: daysAgo(now, 90, 0, 0).toISOString().split('T')[0],
      medicationExpiryDate: ninetyDaysFromNowForMedication,
      orderExpiryDate: thirtyDaysFromNowForMedication,
      lastReconciledAt: daysAgo(now, 30, 0, 0).toISOString().split('T')[0],
      nextReconciliationDue: now.toISOString().split('T')[0],
      ...
      status: 'Available',
      notes: 'Reconciliation due today during Maria Johnson morning visit.',
      familyVisible: false,
      deletedAt: null,
      createdAt: daysAgo(now, 10, 0, 0),
      updatedAt: daysAgo(now, 1, 0, 0),
    },
    {
      ...
      prescriberName: 'Dr. Elena Morris',
      pharmacyName: 'Silver Spring Family Pharmacy',
      startDate: daysAgo(now, 120, 0, 0).toISOString().split('T')[0],
      medicationExpiryDate: oneHundredTwentyDaysFromNow,
      orderExpiryDate: fortyFiveDaysFromNow,
      lastReconciledAt: daysAgo(now, 15, 0, 0).toISOString().split('T')[0],
      nextReconciliationDue: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      quantityAvailable: 3,
      ...
      nurseApprovalId: null,
      status: 'Low Stock',
      notes: 'Low stock. Refill request should be confirmed before the next week of visits.',
      familyVisible: false,
      deletedAt: null,
      createdAt: daysAgo(now, 10, 0, 0),
      updatedAt: daysAgo(now, 0, 8, 0),
    },
    {
      ...
      form: 'pen',
      route: 'subcutaneous',
      dose: 'Per nursing order',
      frequency: 'Nightly',
      purpose: 'Blood sugar support',
      prescriberName: 'Dr. Elena Morris',
      pharmacyName: 'Silver Spring Family Pharmacy',
      startDate: daysAgo(now, 45, 0, 0).toISOString().split('T')[0],
      medicationExpiryDate: twentyDaysFromNow,
      orderExpiryDate: twentyDaysFromNow,
      lastReconciledAt: daysAgo(now, 14, 0, 0).toISOString().split('T')[0],
      nextReconciliationDue: now.toISOString().split('T')[0],
      quantityAvailable: 2,
      ...
      nurseApprovalId: insulinApprovalId,
      status: 'Needs Nurse Review',
      notes: 'High-risk refrigerated medication. Caregiver reminder only; nurse review required before readiness clears.',
      familyVisible: false,
      deletedAt: null,
      createdAt: daysAgo(now, 8, 0, 0),
      updatedAt: daysAgo(now, 0, 8, 15),
    },
    {
      _id: medicationIds.acetaminophen,
      agencyId,
      branchId: northsideBranchId,
      clientId: clients[0]._id,
      visitId: todayVisits[0]._id,
      ...
      route: 'oral',
      dose: '1 tablet',
      frequency: 'As needed per order',
      purpose: 'Pain support',
      prescriberName: 'Dr. Elena Morris',
      pharmacyName: 'Silver Spring Family Pharmacy',
      startDate: daysAgo(now, 180, 0, 0).toISOString().split('T')[0],
      medicationExpiryDate: yesterday,
      orderExpiryDate: ninetyDaysFromNowForMedication,
      lastReconciledAt: daysAgo(now, 45, 0, 0).toISOString().split('T')[0],
      nextReconciliationDue: now.toISOString().split('T')[0],
      quantityAvailable: 12,
      minimumRequiredQuantity: 4,
      storageRequirement: 'Room temperature',
      isHighRisk: false,
      requiresNurseReview: false,
      nurseApprovalId: null,
      status: 'Expired',
      notes: 'Expired medication should be removed and replaced before caregiver reminder task.',
      familyVisible: false,
      deletedAt: null,
      createdAt: daysAgo(now, 30, 0, 0),
      updatedAt: daysAgo(now, 0, 7, 45),
    },
    {
      _id: medicationIds.davidAtorvastatin,
      agencyId,
      branchId: westviewBranchId,
      clientId: clients[1]._id,
      visitId: todayVisits[1]._id,
      carePlanId: null,
      ...
      prescriberName: 'Dr. Samuel Reed',
      pharmacyName: 'Arlington Pharmacy',
      startDate: daysAgo(now, 60, 0, 0).toISOString().split('T')[0],
      medicationExpiryDate: oneHundredTwentyDaysFromNow,
      orderExpiryDate: fortyFiveDaysFromNow,
      lastReconciledAt: daysAgo(now, 10, 0, 0).toISOString().split('T')[0],
      nextReconciliationDue: new Date(Date.now() + 20 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      ...
      familyVisible: false,
      deletedAt: null,
      createdAt: daysAgo(now, 14, 0, 0),
      updatedAt: daysAgo(now, 2, 0, 0),
    },
    {
      _id: medicationIds.helenDonepezil,
      agencyId,
      branchId: northsideBranchId,
      clientId: clients[2]._id,
      visitId: todayVisits[2]._id,
      carePlanId: null,
      ...
      prescriberName: 'Dr. Nina Patel',
      pharmacyName: 'Rockville Care Pharmacy',
      startDate: daysAgo(now, 75, 0, 0).toISOString().split('T')[0],
      medicationExpiryDate: ninetyDaysFromNowForMedication,
      orderExpiryDate: thirtyDaysFromNowForMedication,
      lastReconciledAt: daysAgo(now, 40, 0, 0).toISOString().split('T')[0],
      nextReconciliationDue: yesterday,
      quantityAvailable: 9,
      minimumRequiredQuantity: 7,
      storageRequirement: 'Room temperature',
      isHighRisk: false,
      ...
      nurseApprovalId: null,
      status: 'Needs Refill',
      notes: 'Reconciliation overdue and refill should be confirmed.',
      familyVisible: false,
      deletedAt: null,
      createdAt: daysAgo(now, 21, 0, 0),
      updatedAt: daysAgo(now, 1, 0, 0),
    },
    {
      _id: medicationIds.samuelWarfarin,
      agencyId,
      branchId: westviewBranchId,
      clientId: clients[3]._id,
      visitId: todayVisits[3]._id,
      carePlanId: null,
      ...
      dose: 'Per order',
      frequency: 'Daily',
      purpose: 'Anticoagulation support',
      prescriberName: 'Dr. Samuel Reed',
      pharmacyName: 'Arlington Pharmacy',
      startDate: daysAgo(now, 90, 0, 0).toISOString().split('T')[0],
      medicationExpiryDate: fortyFiveDaysFromNow,
      orderExpiryDate: twentyDaysFromNow,
      lastReconciledAt: daysAgo(now, 7, 0, 0).toISOString().split('T')[0],
      nextReconciliationDue: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      quantityAvailable: 5,
      minimumRequiredQuantity: 5,
      storageRequirement: 'Room temperature',
      isHighRisk: true,
      requiresNurseReview: true,
      nurseApprovalId: null,
      status: 'Needs Nurse Review',
      notes: 'High-risk medication requires nurse confirmation before readiness clears.',
      familyVisible: false,
      deletedAt: null,
      createdAt: daysAgo(now, 18, 0, 0),
      updatedAt: daysAgo(now, 0, 9, 0),
    },
  ]);
  ...
  await connection.collection('medicalAvailability').insertMany([
    ...
    },
    {
      ...
      clientId: clients[0]._id,
      visitId: todayVisits[0]._id,
      sourceMedicationId: medicationIds.lisinopril,
      serviceType: 'Medication: Lisinopril',
      status: 'unavailable',
      scheduledDate: now.toISOString().split('T')[0],
      providerName: 'Medication Management',
      notes: 'Low Stock: Lisinopril 10mg tablet has 3 available; minimum required is 7.',
      confirmedAt: null,
      deletedAt: null,
      createdAt: daysAgo(now, 0, 8, 0),
      updatedAt: daysAgo(now, 0, 8, 0),
    },
    {
      ...
      visitId: todayVisits[0]._id,
      sourceMedicationId: medicationIds.insulin,
      serviceType: 'Medication: Insulin Glargine',
      status: 'unavailable',
      scheduledDate: now.toISOString().split('T')[0],
      providerName: 'Medication Management',
      notes: 'Needs Nurse Review: high-risk refrigerated medication blocks medication readiness.',
      confirmedAt: null,
      deletedAt: null,
      createdAt: daysAgo(now, 0, 8, 15),
      updatedAt: daysAgo(now, 0, 8, 15),
    },
    {
      agencyId,
      ...
      visitId: todayVisits[0]._id,
      sourceMedicationId: medicationIds.acetaminophen,
      serviceType: 'Medication: Acetaminophen',
      status: 'unavailable',
      scheduledDate: now.toISOString().split('T')[0],
      providerName: 'Medication Management',
      notes: 'Expired: Acetaminophen 500mg expired yesterday.',
      confirmedAt: null,
      deletedAt: null,
      createdAt: daysAgo(now, 0, 7, 45),
      updatedAt: daysAgo(now, 0, 7, 45),
    },
  ]);
  ...
  await connection.collection('expirationRecords').insertMany([
    ...
    },
    {
      ...
      caregiverId: clients[0]._id,
      sourceMedicationId: medicationIds.acetaminophen,
      documentType: 'Medication Expiration: Acetaminophen',
    ...
    },
    {
      ...
      caregiverId: clients[0]._id,
      sourceMedicationId: medicationIds.insulin,
      documentType: 'Medication Order: Insulin Glargine',
  ...
  return {
    ...
    counts: {
      ...
      nurseApprovals: 4,
      inspectionRules: 6,
      inspectionFindings: 5,
      socialWorkCases: 3,
      intakeRecords: 3,
      medicalAvailability: 6,
      expirationRecords: 6,
      medicationRecords: 7,
    },
    dashboard: {
